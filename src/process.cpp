...
int *order_count;
int *system_time;
int recRiderLocation[RIDER_NUM+1][2] = {-1};
...
void rider_process(int id) 
{
    ...
    while (true) {
        ...
        rider.self_id = id;
        rider.self_x = recRiderLocation[id][0];
        rider.self_y = recRiderLocation[id][1]; 
...
void create_rider_process(int l, int r, std::string outOutDir) 
{
    pid_t pid;
    std::ifstream pFile(outOutDir);
    std::stringstream buffer;
    buffer << pFile.rdbuf();
    std::string line;
    int riderId, riderX, riderY;
    while (std::getline(pFile, line)) {
        if(line.find("rider") == 0) {
            if (sscanf(line.c_str(), "rider%d%d%d", &riderId, &riderX, &riderY) == 3) {
                recRiderLocation[riderId][0]=riderX;
                recRiderLocation[riderId][1]=riderY;
            }
        }
    }
    pFile.close();
